...
import { PET_BODY_OPTIONS } from '../../core/config.js?v=62';
...
export function init() {
    ...
    if (btnBack) btnBack.onclick = back;
...
async function refreshGroupDetail() {
    ...
    try {
        const { fetchGroupDetail } = await import('../../features/groups/controller.js?v=62');
...
async function handleRefreshInvite() {
    ...
    try {
        const { refreshGroupInviteCode } = await import('../../features/groups/controller.js?v=62');
...
function renderMembers(g) {
    ...
    ul.querySelectorAll('.btn-rm-member').forEach(btn => {
        btn.onclick = async () => {
            ...
            try {
                const { removeGroupMember } = await import('../../features/groups/controller.js?v=62');
...
async function handleSaveName() {
    ...
    try {
        const { createGroup, updateGroupName } = await import('../../features/groups/controller.js?v=62');
        if (!currentGroupId) {
...
async function handleAddMember() {
    ...
    try {
        const { addGroupMember } = await import('../../features/groups/controller.js?v=62');
...
async function handleSavePetBody() {
    ...
    if (!currentGroupId) return;
    try {
        const { updatePet } = await import('../../features/groups/controller.js?v=62');
