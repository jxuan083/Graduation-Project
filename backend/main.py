...
def _get_meeting_or_403(meeting_id: str, uid: str, require_host: bool = False) -> dict:
    """
    取得聚會資料：先試 rooms（進行中），再試 meetings（已結束）。
    聚會 room_id 和結束後的 meeting_id 是同一個，所以兩邊都用同一個 id 查。
    失敗直接 raise HTTPException。
    """
    # 1. 進行中的聚會（在 rooms collection）
    room_doc = db.collection("rooms").document(meeting_id).get()
    if room_doc.exists:
        data = room_doc.to_dict() or {}
        host_uid = data.get("host_uid")
        members_dict = data.get("members") or {}
        # rooms 的 members 是 dict（key 為 uid），房主可能不在裡面
        is_member = (uid == host_uid) or (uid in members_dict)
        if not is_member:
            raise HTTPException(status_code=403, detail="你沒有參與這場聚會")
        if require_host and host_uid != uid:
            raise HTTPException(status_code=403, detail="只有房主可以執行此操作")
        return data

    # 2. 已結束的聚會（在 meetings collection）
