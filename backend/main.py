...
# ── 啟動時預載 dlib 模型（跑在 thread，不阻塞 event loop）──
...
async def pet_swap_echo(
    ...
    """診斷用：只讀取檔案然後回傳 1x1 白色 JPEG，不做 face_swap"""
    await image.read()
    ...
    return Response(content=tiny, media_type="image/jpeg")
...
async def pet_swap(
    ...
    try:
        def _do_swap(in_path, a_path, out_path):
            from face_swap import face_swap as run_face_swap
            return run_face_swap(in_path, a_path, out_path)

        ok = await asyncio.to_thread(_do_swap, tmp_in_path, animal_path, tmp_out_path)
